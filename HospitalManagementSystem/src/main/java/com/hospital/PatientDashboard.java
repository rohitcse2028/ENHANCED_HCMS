...
public class PatientDashboard extends JFrame {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (dateField.getText().trim().isEmpty() || 
            ((String)timeCombo.getSelectedItem()).isEmpty()) {
            JOptionPane.showMessageDialog(PatientDashboard.this, 
                "Please select date and time!");
            return;
        }
        
        // ATOMIC TRANSACTION USE KARO
        boolean success = TransactionManager.executeTransaction((Connection conn) -> {
            // Step 1: Check if slot available (WITH LOCK)
            String checkQuery = "SELECT * FROM appointment_slots WHERE " +
                               "doctor_id = ? AND appointment_date = ? AND " +
                               "appointment_time = ? FOR UPDATE"; // LOCK the row
            
            PreparedStatement checkStmt = conn.prepareStatement(checkQuery);
            checkStmt.setString(1, getDoctorIdFromName((String)doctorCombo.getSelectedItem()));
            checkStmt.setString(2, dateField.getText().trim());
            checkStmt.setString(3, (String)timeCombo.getSelectedItem() + ":00");
            
            ResultSet rs = checkStmt.executeQuery();
            if (rs.next() && "BOOKED".equals(rs.getString("status"))) {
                throw new SQLException("This time slot is already booked!");
            }
            rs.close();
            checkStmt.close();
            
            // Step 2: Generate appointment ID
            String appointmentId = "A" + String.format("%03d", getNextAppointmentId(conn));
            ...
            insertStmt.setString(1, appointmentId);
            insertStmt.setString(2, patientId);
            insertStmt.setString(3, getDoctorIdFromName((String)doctorCombo.getSelectedItem()));
            insertStmt.setString(4, dateField.getText().trim());
            insertStmt.setString(5, (String)timeCombo.getSelectedItem() + ":00");
            insertStmt.setString(6, (String)typeCombo.getSelectedItem());
            
            insertStmt.executeUpdate();
            insertStmt.close();
            
            // Step 4: Mark slot as booked
            String slotQuery = "INSERT INTO appointment_slots " +
            ...
            slotStmt.setString(1, getDoctorIdFromName((String)doctorCombo.getSelectedItem()));
            slotStmt.setString(2, dateField.getText().trim());
            slotStmt.setString(3, (String)timeCombo.getSelectedItem() + ":00");
        ...
        if (success) {
            JOptionPane.showMessageDialog(PatientDashboard.this,
                ...
                "Appointment ID: A" + String.format("%03d", getNextAppointmentIdStatic()) + "\n" +
    ...
    // PatientDashboard.java mein ye method add karo
private int getNextAppointmentId(Connection conn) throws SQLException {
    try (Statement stmt = conn.createStatement()) {
        ResultSet rs = stmt.executeQuery(
            "SELECT MAX(CAST(SUBSTRING(appointment_id, 2) AS UNSIGNED)) FROM appointments");
        int maxId = 0;
        if (rs.next()) {
            maxId = rs.getInt(1);
        }
...
// Aur ye bhi add karo for other uses
private static int getNextAppointmentIdStatic() {

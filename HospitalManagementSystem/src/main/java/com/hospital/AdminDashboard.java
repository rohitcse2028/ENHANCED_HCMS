   ...
   private void loadUserData() {
    try {
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            java.util.List<UserEntity> users = userDAO.getAllUsers();
    ...
    if (result == JOptionPane.OK_OPTION) {
        ...
        try {
            TransactionManager.executeTransaction((Connection conn) -> {
                ...
                // If doctor or patient, create corresponding record
                String role = (String) roleCombo.getSelectedItem();
                if ("Doctor".equalsIgnoreCase(role)) {
                    createDoctorRecord(conn, userId, nameField.getText().trim());
                } else if ("Patient".equalsIgnoreCase(role)) {
                    createPatientRecord(conn, userId, nameField.getText().trim());
                }
            });
...
private void createDoctorRecord(Connection conn, String userId, String name) throws SQLException {
    String doctorId = "D" + String.format("%03d", getNextDoctorId(conn));
    String query = "INSERT INTO doctors (doctor_id, user_id, name, specialization, status) " +
...
private void createPatientRecord(Connection conn, String userId, String name) throws SQLException {
    String patientId = "P" + String.format("%03d", getNextPatientId(conn));
    String query = "INSERT INTO patients (patient_id, user_id, name, date_of_birth, gender) " +
                  "VALUES (?, ?, ?, CURDATE(), 'Unknown')";
...
private int getNextDoctorId(Connection conn) throws SQLException {
    String query = "SELECT MAX(CAST(SUBSTRING(doctor_id, 2) AS UNSIGNED)) FROM doctors";
...
private int getNextPatientId(Connection conn) throws SQLException {
...
int result = JOptionPane.showConfirmDialog(this, panel, "Add New Appointment",
    ...
    if (result == JOptionPane.OK_OPTION) {
        ...
        try {
            ...
            String query = "INSERT INTO appointments (appointment_id, patient_id, " +
                "doctor_id, appointment_date, appointment_time, type, status) " +
                "VALUES (?, ?, ?, ?, ?, ?, 'Scheduled')";
